...
async function readFrame(stream) {
    ...
    for await (const chunk of stream) {
        ...
        if (acc.byteLength >= 4) {
            ...
            if (acc.byteLength >= 4 + len)
                return u8ToString(acc.subarray(4, 4 + len));
        }
    }
...
async function sendAll(stream, bytes) {
    for (let off = 0; off < bytes.length; off += MAX_SEND) {
        ...
        if (stream.send(chunk) === false && typeof stream.onDrain === 'function')
...
export async function serveResolve(vl, handler) {
    ...
    await vl.node.handle(RESOLVE_PROTO, (stream, _connection) => {
        void (async () => {
            try {
                ...
                try {
                    stream.abort?.(new Error('resolve handler error'));
                }
                catch { }
            }
        })();
 ...
 */
export async function resolveViaLibp2p(vl, target, pointer) {
    const dialTarget = target.startsWith('/') ? multiaddr(target) : peerIdFromString(target);
    const stream = await vl.node.dialProtocol(dialTarget, RESOLVE_PROTO, { runOnLimitedConnection: true });
...
export async function serveResolveHttp(vl, handler) {
    const httpSvc = vl.node.services.http;
    if (httpSvc == null)
        throw new Error('http() service not enabled on this vessel — pass enableHttp:true');
    ...
    try {
        const reg = vl.node.components?.registrar ?? vl.node.registrar;
 ...
 */
export async function resolveViaHttp(vl, target, pointer) {
    const httpSvc = vl.node.services.http;
    if (httpSvc == null)
        throw new Error('http() service not enabled on this vessel — pass enableHttp:true');
    const dialTarget = target.startsWith('/') ? multiaddr(target) : peerIdFromString(target);
    ...
    try {
        await vl.node.dial(dialTarget);
    }
    catch { /* fetch may still establish it */ }
    ...
    const res = await httpSvc.fetchProtocol(dialTarget, RESOLVE_HTTP_PROTO, {
        method: 'POST',
...
export async function createVesselLibp2p(opts) {
    ...
    if (opts.relayMultiaddr) {
        ...
        const dialRelay = async () => { try {
            await node.dial(relayMa);
        }
        catch { /* reservation retries internally */ } };
        await dialRelay();
        ...
        refreshTimer = setInterval(() => {
            ...
            if (relayConns.length === 0) {
                void dialRelay().then(() => { lastReservationAt = Date.now(); });
                return;
            }
        ...
        node.addEventListener('connection:close', (evt) => {
            try {
                ...
                if (relayPeer && evt?.detail?.remotePeer?.toString() === relayPeer)
                    void dialRelay();
            }
            catch { /* ignore */ }
        });
    ...
    return {
        ...
        stop: async () => { if (refreshTimer)
            clearInterval(refreshTimer); await node.stop(); },
    };
}
...
function healthSnapshot(node) {
    ...
    const secs = connections.map((c) => c.secondsRemaining).filter((s) => s != null);
    ...
    try {
        ...
        for (const [k, v] of Object.entries(snap)) {
            ...
            if (/dcutr.*(success|connected)/i.test(k))
                holePunchSuccess += val;
            else if (/dcutr.*(fail|error)/i.test(k))
                holePunchFail += val;
            else if (/(stream).*(reset|abort)/i.test(k))
                streamResets += val;
        }
    }
