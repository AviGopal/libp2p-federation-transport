...
import { createVesselLibp2p, serveResolveHttp } from './index.ts';
...
function die(msg) { console.error('[fed-sidecar] ERROR:', msg); process.exit(1); }
if (!VESSEL_ID)
    die('set FED_VESSEL_ID');
if (!DISCOVERY)
    die('set DISCOVERY_URL');
if (!LOCAL_RESOLVE_URL)
    die('set LOCAL_RESOLVE_URL (the local vessel /resolve to proxy to)');
if (SHAPES.length === 0)
    die('set FED_SHAPES (comma-separated shapes to advertise)');
...
if (!RELAY) {
    try {
        ...
        console.warn(`[fed-sidecar] bootstrap fetch failed (${String(e?.message ?? e)})`);
    }
}
if (!RELAY)
    die('set RELAY_MULTIADDR, or DISCOVERY_URL must expose relay_multiaddrs via /bootstrap');
...
const vl = await createVesselLibp2p({ vesselId: VESSEL_ID, relayMultiaddr: RELAY, enableHttp: true });
...
await serveResolveHttp(vl, async (pointer) => {
    try {
        ...
        if (j == null)
            return { error: 'local vessel returned non-JSON' };
        if (j.success === false)
            return { error: j.error || 'local vessel resolve failed' };
        // Prefer {content}; fall back to {body}; else the raw payload.
        return 'content' in j ? j.content : ('body' in j ? j.body : j);
    }
    catch (e) {
        return { error: 'sidecar proxy to local vessel failed: ' + String(e?.message ?? e) };
    }
});
...
if (!circuit)
    console.warn('[fed-sidecar] no circuit multiaddr yet — relay reservation pending');
