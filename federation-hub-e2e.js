...
let circuit = '';
for (let i = 0; i < 40; i++) {
    const c = ingress.advertiseMultiaddrs().find((m) => m.includes('p2p-circuit'));
    if (c) {
        circuit = c;
        break;
    }
...
if (!circuit) {
    console.error('FAIL: ingress got no circuit reservation on the syzygy relay');
    process.exit(1);
}
...
if (!reg.ok) {
    console.error('FAIL: hub register HTTP ' + reg.status);
    process.exit(1);
}
...
if (!v || v.protocol !== 'libp2p' || !v.libp2p_multiaddr?.[0]) {
    console.error('FAIL: hub discovery did not return the spoke as libp2p', JSON.stringify(dj).slice(0, 300));
    process.exit(1);
}
...
for (let i = 0; i < 20; i++) {
    if (egress.advertiseMultiaddrs().some((m) => m.includes('p2p-circuit')))
        break;
    await new Promise((r) => setTimeout(r, 500));
}
const res = await resolveViaHttp(egress, v.libp2p_multiaddr[0], { type: SHAPE });
...
await egress.stop().catch(() => { });
if (value === VALUE) {
    console.log('\nFEDERATION E2E PASS — local spoke resolved through the public syzygy relay');
    process.exit(0);
}
console.error('\nFAIL: unexpected resolve value');
process.exit(1);
//# sourceMappingURL=federation-hub-e2e.js.map